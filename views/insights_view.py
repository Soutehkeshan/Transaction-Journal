...
class InsightsView(QWidget):
    def __init__(self):
        ...
        for btn in [self.most_asset_btn, self.least_asset_btn, self.most_btc_btn, self.least_btc_btn, self.most_gold_btn, self.least_gold_btn]:
            button_layout.addWidget(btn)

        # Table
        self.table = QTableWidget()
        self.table.setColumnCount(6)
        self.table.setHorizontalHeaderLabels(["Asset", "Amount", "Buy Price", "Gain", "BTC Gain", "Gold Gain"])
    ...
    def update_table(self, transactions):
        ...
        for row, tx in enumerate(transactions):
            ...
            self.table.setItem(row, 3, QTableWidgetItem(f"{tx.gain:.2f}"))
            self.table.setItem(row, 4, QTableWidgetItem(f"{tx.btc_gain:.6f}"))
            self.table.setItem(row, 5, QTableWidgetItem(f"{tx.gold_gain:.6f}"))
